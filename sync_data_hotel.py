...
TARGET_SHEET_NAME = "호텔상품리스트"

# 날짜 포맷팅 (예: 2026년 8월 -> '26년 8월')
now = datetime.now()
YEAR_SHORT = now.strftime("%y")  # '26'
CURRENT_MONTH_NUM = now.month    # 8
FOLDER_TARGET_NAME = f"{YEAR_SHORT}년 {CURRENT_MONTH_NUM}월(호텔)"

print(f"⏰ '{FOLDER_TARGET_NAME}' 데이터 동기화를 시작합니다.")

# 1. '26년 8월(호텔)' 형식의 폴더 찾기
query = f"mimeType = 'application/vnd.google-apps.folder' and name contains '{FOLDER_TARGET_NAME}'"
results = drive_service.files().list(q=query, spaces='drive', fields='files(id, name)').execute()
folders = results.get('files', [])

if not folders:
    print(f"❌ '{FOLDER_TARGET_NAME}' 폴더를 찾을 수 없습니다.")
    exit()

folder_id = folders[0]['id']

# 2. 폴더 내에서 가장 최신 'HntNaverEP_hotel_*.xlsx' 파일 찾기
file_query = f"'{folder_id}' in parents and name contains 'HntNaverEP_hotel_' and mimeType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'"
...
# 대용량 처리를 위해 데이터를 행 단위로 쪼개어 대량 업로드
CHUNK_SIZE = 10000  # 1만 행씩 나누어 전송
print(f"🚀 {len(values):,}행 데이터를 {CHUNK_SIZE:,}행씩 분할 업로드 시작...")

# 첫 번째 청크에는 헤더 포함
first_chunk = [header] + values[:CHUNK_SIZE-1]
worksheet.update(first_chunk, 'A1')
...
for i in range(CHUNK_SIZE - 1, len(values), CHUNK_SIZE):
    ...
    worksheet.update(chunk, range_start)
    start_row += len(chunk)
